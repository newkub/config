...
jobs:
  release:
    ...
    steps:
      ...
      - run: bun install
      - name: Release
        run: bun run release-it ${{ github.event.inputs.version }} --ci
        env:
...
- **Optimize Caching**: Add `.turbo` to `.gitignore` and use remote caching in CI
- **Shared Configurations**: Create packages for shared configs (TypeScript, ESLint)
- **Workspace Dependencies**: Use `"workspace:*"` for internal dependencies
- **Conventional Commits**: Follow the format `type(scope): message` for automatic changelogs
- **Git Hooks**: Set up pre-commit hooks for linting and formatting